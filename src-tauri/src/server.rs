...
#[tauri::command]
pub async fn get_server_info(connection: State<'_, GrpcConnection>) -> Result<ServerInfoPayload, String> {
...
#[tauri::command]
pub async fn get_server_logs(connection: State<'_, GrpcConnection>) -> Result<ServerLogsPayload, String> {
  let mut guard = connection.connection.lock().await;
  if let Some(ref mut connection) = *guard {
